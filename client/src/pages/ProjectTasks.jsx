...
export default function ProjectTasks() {
  ...
  return (
    <div>
      ...
      {/* Mensajes globales del backend (incluye 400, 404, etc.) */}
      {serverList.length > 0 && (
      ...
      <form onSubmit={save} noValidate className="mb-6 grid gap-2 max-w-lg">
        <div>
          <input
            ...
            name="title"
            value={form.title}
            onChange={e=>{ setForm(f=>({...f, title:e.target.value})); if (fieldErrors.title) setFieldErrors(fe=>({...fe, title:undefined})); }}
